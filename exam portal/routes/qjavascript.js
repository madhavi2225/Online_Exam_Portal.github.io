...
router.get('/', function (req, res, next) {
    ...
    if (sess.userId && sess.admin) {
        db.query(`Select * from qjavascript ORDER BY id ASC`, (err, result) => {
            if (!err) {
                // res.send(result.rows);
                console.log(result.rows);
                res.render('qjavascript', { userData: result.rows })
            }
            else {
...
router.post('/create', function (req, res, next) {
    db.query('SELECT q from qjavascript where q =$1', [req.body.q], (err, result) => {
        ...
        let user = req.body;
        let insertQuery = `insert into qjavascript(q, a, b, c, d,ans) 
