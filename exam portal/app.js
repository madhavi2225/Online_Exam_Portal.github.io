...
app.post('/uphtml/:id', function (req, res, next) {
    let d = req.body
    let flag = false;
    db.query('SELECT q from qhtml WHERE id != $1', [req.params.id], (err, result) => {
        for (let i = 0; i < result.rows.length; i++) {
            if (result.rows[i].q == d.q) {
                flag = false;
                alert("Que already exicst");
                break;
            } else {
                flag = true;
                console.log('update');
...
app.post('/upcss/:id', function (req, res, next) {
    db.query('SELECT q from qcss ', (err, result) => {
        if (true) {
            if (result.rowCount > 0) {
...
app.post('/upjs/:id', function (req, res, next) {
    db.query('SELECT q from qjavascript ', (err, result) => {
        if (true) {
            if (result.rowCount > 0) {
                if (result.rows[0].q == req.body.q) {
                    fe = true;
